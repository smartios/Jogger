...
-(NSString *)filePath
{
    ...
    NSLog(@"File Path of Database = %@",[documentsDir stringByAppendingPathComponent:@"janeous.db"]);
    
    
    return [documentsDir stringByAppendingPathComponent:@"janeous.db"];
}
...
-(NSMutableArray *)getChatList
{
    ...
    NSArray *keys = [[NSArray alloc] initWithObjects:@"read_no",@"unread_no",@"user_image",@"user_name", @"name",@"user_id",@"last_login_time",@"id",@"favorite",@"last_message",@"sender_id",@"receiver_id", @"last_message_time",@"message_id",@"attachment_type",@"group_id",@"group_name",@"group_icon",@"group_type", nil];
    
    
    //[self openDB];
    
    NSString *qsql = [NSString stringWithFormat:@"SELECT read_no, unread_no, uu.user_image,uu.user_name, uu.name, uu.id as user_id, uu.last_login_time,ucl.id as id, ucl.favorite, message as last_message, sender_id, receiver_id, msg.last_message_time, message_id, attachment_type, ucl.group_id, ug.group_name, ug.group_icon, ug.group_type, read_status, custom_unique_key FROM uma_chat_list as ucl LEFT JOIN ( SELECT SUM( CASE WHEN( read_status = 'unread' AND (receiver_id = %@ OR (group_id <> ''  AND sender_id != %@) ) ) THEN 1 ELSE 0 END ) AS unread_no, SUM( CASE WHEN( read_status = 'read' AND (sender_id = %@ OR group_id <> '' ) ) THEN 1 ELSE 0 END ) AS read_no, sender_id, receiver_id, message, created_at as last_message_time, message_id, read_status, attachment_type, group_id, CASE WHEN group_id <> '' THEN group_id ELSE sender_id + receiver_id END as custom_unique_key FROM `uma_messages` GROUP BY CASE WHEN group_id <> '' THEN group_id ELSE sender_id + receiver_id END ) as msg ON CASE WHEN ucl.group_id <> '' THEN (msg.group_id = ucl.group_id ) ELSE ((msg.sender_id = %@ OR msg.receiver_id = %@) AND (ucl.connected_user_id = msg.sender_id OR ucl.connected_user_id = msg.receiver_id ) and msg.group_id = '') END LEFT JOIN uma_groups as ug ON ug.id = ucl.group_id LEFT JOIN uma_users as uu ON uu.id = ucl.connected_user_id WHERE ucl.id <> '' AND (ug.group_type <> \"CWA\" OR ucl.group_id = '') GROUP BY ucl.id order BY msg.last_message_time DESC",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]];
    
    //NSString *qsql = [NSString stringWithFormat:@"SELECT unread_no, read_no,uu.user_image,uu.name,uu.user_id,uu.last_login_time, ucl.id as chat_id,ucl.favorite, um.message,um.sender_id,um.receiver_id,um.created_at,um.message_id,um.attachment_type FROM `uma_chat_list` as ucl LEFT JOIN `uma_users` as uu ON uu.user_id = ucl.connected_user_id LEFT JOIN ( select SUM (CASE WHEN (read_status='unread'  and receiver_id = %@ )THEN 1 ELSE 0 END ) as unread_no, SUM (CASE WHEN (read_status='read' and sender_id = %@) THEN 1 ELSE 0 END ) as read_no, sender_id,receiver_id,message,created_at,message_id,read_status,attachment_type from `uma_messages` GROUP BY sender_id+receiver_id ORDER BY created_at ASC   ) as um ON (ucl.connected_user_id = um.sender_id  AND um.receiver_id = %@ ) OR (ucl.connected_user_id = um.receiver_id  AND um.sender_id = %@ )  where ucl.user_id = %@ AND ucl.connected_user_id != 1 GROUP BY um.sender_id+um.receiver_id  order BY um.created_at DESC", [[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]];
    
    //NSString *qsql = [NSString stringWithFormat:@"SELECT unread_no,read_no,uu.user_image,uu.name,uu.user_id,uu.last_login_time, ucl.id as chat_id,ucl.favorite, um.message,um.sender_id,um.receiver_id,um.created_at,um.message_id,um.attachment_type , ug.id as group_id,ug.group_name, ug.group_icon,ug.group_type FROM `uma_chat_list` as ucl LEFT JOIN `uma_users` as uu ON uu.user_id = ucl.connected_user_id LEFT JOIN ( select SUM (CASE WHEN (read_status='unread'  and receiver_id = %@ )THEN 1 ELSE 0 END ) as unread_no, SUM (CASE WHEN (read_status='read' and sender_id = %@) THEN 1 ELSE 0 END ) as read_no, sender_id,receiver_id,message,created_at,message_id,read_status,attachment_type,group_id from `uma_messages` GROUP BY sender_id+receiver_id ORDER BY created_at ASC) as um ON (ucl.connected_user_id = um.sender_id  AND um.receiver_id = %@ ) OR (ucl.connected_user_id = um.receiver_id  AND um.sender_id = %@ ) OR um.group_id  = ucl.group_id LEFT JOIN `uma_groups` as ug ON ug.id = ucl.group_id where (ucl.user_id = %@ OR ug.id IS NOT NULL ) AND ucl.connected_user_id != 1 GROUP BY ucl. id order BY um.created_at DESC", [[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]];
    
  //  NSString *qsql = [NSString stringWithFormat:@"SELECT  uu.user_image,uu.name,uu.user_id,uu.last_login_time, ucl.id as chat_id,ucl.favorite, um.message,um.sender_id,um.receiver_id,um.created_at,um.message_id,um.attachment_type , ug.id as group_id,ug.group_name, ug.group_icon,ug.group_type FROM `uma_chat_list` as ucl LEFT JOIN `uma_users` as uu ON uu.user_id = ucl.connected_user_id LEFT JOIN `uma_groups` as ug ON ug.id = ucl.group_id LEFT JOIN `uma_messages` as um ON (CASE WHEN (ucl.group_id != \"\" AND ucl.group_id = ug.id )THEN um.group_id = ug.id ELSE (um.receiver_id = ucl.user_id OR um.sender_id = ucl.user_id) END) WHERE ucl.user_id = \"%@\" GROUP BY ucl.id order BY um.created_at DESC",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]];
...
{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSArray *keys = [[NSArray alloc] initWithObjects:@"read_no",@"unread_no",@"user_image", @"name",@"user_id",@"last_login_time",@"id",@"favorite",@"last_message",@"sender_id",@"receiver_id", @"last_message_time",@"message_id",@"attachment_type",@"group_id",@"group_name",@"group_icon",@"group_type", nil];
    
    
    //[self openDB];
    
    NSString *qsql = [NSString stringWithFormat:@"SELECT read_no, unread_no, uu.user_image, uu.name, uu.id as user_id, uu.last_login_time,ucl.id as id, ucl.favorite, message as last_message, sender_id, receiver_id, msg.last_message_time, message_id, attachment_type, ucl.group_id, ug.group_name, ug.group_icon, ug.group_type, read_status, custom_unique_key FROM uma_chat_list as ucl LEFT JOIN ( SELECT SUM( CASE WHEN( read_status = 'unread' AND (receiver_id = %@ OR (group_id <> ''  AND sender_id != %@) ) ) THEN 1 ELSE 0 END ) AS unread_no, SUM( CASE WHEN( read_status = 'read' AND (sender_id = %@ OR group_id <> '' ) ) THEN 1 ELSE 0 END ) AS read_no, sender_id, receiver_id, message, created_at as last_message_time, message_id, read_status, attachment_type, group_id, CASE WHEN group_id <> '' THEN group_id ELSE sender_id + receiver_id END as custom_unique_key FROM `uma_messages` GROUP BY CASE WHEN group_id <> '' THEN group_id ELSE sender_id + receiver_id END ) as msg ON CASE WHEN ucl.group_id <> '' THEN (msg.group_id = ucl.group_id ) ELSE ((msg.sender_id = %@ OR msg.receiver_id = %@) AND (ucl.connected_user_id = msg.sender_id OR ucl.connected_user_id = msg.receiver_id )) END LEFT JOIN uma_groups as ug ON ug.id = ucl.group_id LEFT JOIN uma_users as uu ON uu.id = ucl.connected_user_id WHERE ucl.id <> '' AND ucl.group_id <> \"\" AND ug.group_type <> \"CWA\" GROUP BY ucl.id order BY msg.last_message_time DESC",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]];
...
-(NSMutableArray *)getContactList
{
    ...
    NSString *qsql = [NSString stringWithFormat:@"SELECT * FROM `uma_users` WHERE user_id != \"1\" AND user_id != \"%@\" AND (location_id = %@ OR location_id = '') ORDER BY `name` COLLATE NOCASE ASC",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"location_id"]];
...
-(NSMutableArray *)getGroupRemainingContactList:(NSString *)groupID
{
    ...
    NSString *qsql = [NSString stringWithFormat:@"SELECT uu.id, uu.user_id, uu.name,uu.user_name, uu.mobile, uu.user_image, uu.last_login_time FROM `uma_users` as uu LEFT JOIN `uma_group_members` as ugm ON ugm.group_id = %@ AND ugm.user_id = uu.user_id WHERE uu.user_id != \"1\" AND uu.user_id != \"%@\" AND (location_id = %@ OR location_id = '') AND ugm.group_id is null GROUP BY uu.user_id ORDER BY `name` COLLATE NOCASE ASC",groupID,[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"location_id"]];
...
{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSArray *keys = [[NSArray alloc] initWithObjects:@"unread_no",@"read_no",@"user_id",@"user_name",@"name",@"mobile",@"user_image",@"chat_id",@"favorite",@"last_message",@"attachment_type",@"last_message_time",@"last_message_time", nil];
    
    //[self openDB];
    
    //NSString *qsql = [NSString stringWithFormat:@"uu.user_id, uu.name, uu.mobile,uu.user_image, ucl.id, ucl.favorite,ucl.last_message_time, um.message FROM `uma_users` as uu LEFT JOIN `uma_chat_list` as ucl ON  ucl.connected_user_id = uu.user_id LEFT JOIN `uma_messages` as um ON (um.sender_id = uu.user_id OR um.receiver_id =  uu.user_id) WHERE user_id != \"1\" AND user_id != \"%@\" AND name LIKE \"%%%@%%\" GROUP BY uu.user_id ORDER BY `name` COLLATE NOCASE ASC",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],searchString];
    
    NSString *qsql = [NSString stringWithFormat:@"SELECT unread_no,read_no,uu.user_id,uu.user_name, uu.name, uu.mobile,uu.user_image, ucl.id, ucl.favorite, um.message, um.attachment_type,um.created_at as last_message_time FROM `uma_users` as uu LEFT JOIN `uma_chat_list` as ucl ON ucl.connected_user_id = uu.user_id LEFT JOIN ( SELECT SUM( CASE WHEN( read_status = 'unread' AND receiver_id = %@ ) THEN 1 ELSE 0 END ) AS unread_no, SUM( CASE WHEN( read_status = 'read' AND sender_id = %@ ) THEN 1 ELSE 0 END ) AS read_no, sender_id, receiver_id, message, created_at, message_id, read_status, attachment_type FROM `uma_messages` WHERE group_id ='' and sender_id <> '' and receiver_id <> '' GROUP BY (sender_id + receiver_id) ) as um ON uu.user_id = um.sender_id OR um.receiver_id = uu.user_id WHERE uu.user_id != \"1\" AND uu.user_id <> %@ AND (user_name LIKE \"%%%@%%\" OR name LIKE \"%%%@%%\") GROUP BY uu.user_id ORDER BY last_message_time DESC, name asc",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],searchString,searchString];
...
{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSArray *keys = [[NSArray alloc] initWithObjects:@"user_image",@"name",@"user_id",@"group_name",@"group_icon",@"group_type",@"group_id",@"sender_id",@"receiver_id",@"last_message",@"last_message_time",@"message_id",@"read_status",@"attachment_type",@"chat_id",@"favorite", nil];
    
    //[self openDB];
    
    NSString *qsql = [NSString stringWithFormat:@"SELECT  uu.user_image,uu.name,uu.user_id,ug.group_name,ug.group_icon,ug.group_type,um.group_id,um.sender_id,um.receiver_id,um.message,um.created_at,um.message_id,um.read_status,um.attachment_type,ucl.id,ucl.favorite from `uma_messages` as um LEFT JOIN `uma_users` as uu ON (CASE WHEN (um.sender_id != \"%@\" )THEN um.sender_id = uu.user_id ELSE um.receiver_id = uu.user_id END ) LEFT JOIN `uma_groups` as ug  ON um.group_id = ug.id LEFT JOIN `uma_chat_list` as ucl ON ucl.group_id = um.group_id  OR (ucl.user_id = uu.user_id OR ucl.connected_user_id = uu.user_id)  WHERE um.message_type != \"action\" AND um.message LIKE \"%%%@%%\" GROUP BY um.message_id",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],searchString];
...
-(NSMutableArray *)getChat:(NSString *)userID orWithGroup:(NSString *)groupID withOffset:(NSInteger)offset
{
    ...
    {
        qsql = [NSString stringWithFormat:@"SELECT um.id,um.mid,um.message_id,um.sender_id,um.message,um.attachment,um.attachment_type,um.message_type,um.group_id,um.receiver_id,um.receiver_time,um.read_status,um.read_at,um.delivery_status,um.delivery_time,um.deleted_at,um.created_at,um.filesize,um.delete_after,um.thumb_name,um.duration,uu.user_image,uu.name,uu.user_color FROM `uma_messages` as um LEFT JOIN `uma_users` as uu ON uu.user_id = um.sender_id WHERE (`receiver_id` = '%@' OR `sender_id` = '%@') AND (`receiver_id` = '%@' OR `sender_id` = '%@') AND `group_id` = ''  ORDER BY `created_at` DESC LIMIT 10 OFFSET %li",userID,userID,[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],(long)offset];
    }
    else
    {
         qsql = [NSString stringWithFormat:@"SELECT um.id,um.mid,um.message_id,um.sender_id,um.message,um.attachment,um.attachment_type,um.message_type,um.group_id,um.receiver_id,um.receiver_time,um.read_status,um.read_at,um.delivery_status,um.delivery_time,um.deleted_at,um.created_at,um.filesize,um.delete_after,um.thumb_name,um.duration,uu.user_image,uu.name,uu.user_color FROM `uma_messages` as um LEFT JOIN `uma_users` as uu ON uu.user_id = um.sender_id WHERE `group_id` = '%@' ORDER BY `created_at` DESC LIMIT 10 OFFSET %li",groupID,(long)offset];
    }
...
-(NSMutableArray *)getWholeChat:(NSString *)userID orWithGroup:(NSString *)groupID
{
    ...
    if([groupID isEqualToString:@""])
    {
        qsql = [NSString stringWithFormat:@"SELECT um.id,um.mid,um.message_id,um.sender_id,um.message,um.attachment,um.attachment_type,um.message_type,um.group_id,um.receiver_id,um.receiver_time,um.read_status,um.read_at,um.delivery_status,um.delivery_time,um.deleted_at,um.created_at,um.filesize,um.delete_after,um.thumb_name,um.duration,uu.user_image,uu.name,uu.user_color FROM `uma_messages` as um LEFT JOIN `uma_users` as uu ON uu.user_id = um.sender_id WHERE (`receiver_id` = '%@' OR `sender_id` = '%@') AND (`receiver_id` = '%@' OR `sender_id` = '%@') AND `group_id` = ''  ORDER BY `created_at` DESC",userID,userID,[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"],[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]];
    }
    else
    {
        qsql = [NSString stringWithFormat:@"SELECT um.id,um.mid,um.message_id,um.sender_id,um.message,um.attachment,um.attachment_type,um.message_type,um.group_id,um.receiver_id,um.receiver_time,um.read_status,um.read_at,um.delivery_status,um.delivery_time,um.deleted_at,um.created_at,um.filesize,um.delete_after,um.thumb_name,um.duration,uu.user_image,uu.name,uu.user_color FROM `uma_messages` as um LEFT JOIN `uma_users` as uu ON uu.user_id = um.sender_id WHERE `group_id` = '%@' ORDER BY `created_at` DESC",groupID];
    }
...
{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    NSArray *keys = [[NSArray alloc] initWithObjects:@"id",@"user_id",@"name",@"user_name",@"mobile",@"user_image",@"last_login_time",@"is_admin", nil];
    
    //[self openDB];
    
    NSString *qsql = [NSString stringWithFormat:@"SELECT uu.id,uu.user_id,uu.name,uu.user_name,uu.mobile,uu.user_image,uu.last_login_time,ugm.is_admin FROM `uma_users` AS uu LEFT JOIN `uma_group_members` AS ugm ON uu.user_id = ugm.user_id  WHERE ugm.group_id = %@",groupID];
...
-(NSMutableArray *)checkIfUserIsInGroup
{
    ...
    NSString *qsql = @"SELECT max(mid) as mid FROM `uma_messages` WHERE mid <> ''";
...
-(NSMutableArray *)getAllUndeliveredMSG
{
    ...
    NSString *qsql = @"SELECT * FROM `uma_messages` WHERE `delivery_status` = \"awaiting\"";
...
-(NSMutableArray *)getAllIncompleteStatusMSG
{
    ...
    NSString *qsql = [NSString stringWithFormat:@"SELECT message_id FROM `uma_messages` WHERE (sender_id = %@) AND ((`delivery_status` = \"awaiting\" OR delivery_status = \"\") OR  (read_status = \"unread\" OR  read_status = \"\") OR (delete_after != \"\"))",[[NSUserDefaults standardUserDefaults] objectForKey: @"user_id"]] ;
...
-(void)insertDataIntoTable:(NSString *)TableName forKeys:(NSArray*)Keys Values:(NSArray *)Values
{
    ...
    if([TableName isEqualToString:@"uma_users"])
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"refresh_user_List" object:nil];
    }
    else if([TableName isEqualToString:@"uma_messages"])
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"newMsg" object:Values];
    }
    else if([TableName isEqualToString:@"uma_groups"] || [TableName isEqualToString:@"uma_group_members"])
    {
