...
#define isIOS7 (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_9_3)
...
- (void)setUpUI {
    ...
    self.frame = CGRectMake(f.origin.x, -height, width, 64);
    ...
    self.img.layer.borderColor=[UIColor clearColor].CGColor;
    
    [self.img.layer setBorderWidth: 0.0];
    ...
    [self.containerView addGestureRecognizer:msgTap];
...
+ (void)showWithNotificationMessage:(NSString *)message completion:(void (^)(void))completion {
    ...
    if (message) {
        ...
        PUSH_VIEW.frame = CGRectMake(f.origin.x, -f.size.height, f.size.width, 62);
        [APP.window addSubview:PUSH_VIEW];
        
        //Show
        [UIView animateWithDuration:SHOW_ANIM_DUR delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            CGRect f = PUSH_VIEW.frame;
            PUSH_VIEW.frame = CGRectMake(f.origin.x, 0, f.size.width, 62);
...
+ (void)closeWitCompletion:(void (^)(void))completion {
    ...
    [UIView animateWithDuration:HIDE_ANIM_DUR delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        CGRect f = PUSH_VIEW.frame;
        PUSH_VIEW.frame = CGRectMake(f.origin.x, -64, f.size.width, 62);
    } completion:^(BOOL finished) {
