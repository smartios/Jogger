...
class Localisator {
    ...
    fileprivate var availableLanguagesArray         = ["zh-Hans","zh-Hant","English_en"]
    ...
    fileprivate func loadDictionaryForLanguage(_ newLanguage:String) -> Bool {
        ...
        // We are typing setting here only, ":$:" have no where in any language code, so zh-Hans or zh-Hant will be automatically assign to arrayExt
        var arrayExt = newLanguage.components(separatedBy: ":$:")
        
        if newLanguage == "English_en"{
        arrayExt = newLanguage.components(separatedBy: "_")
        }
